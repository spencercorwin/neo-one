...
export class Blockchain {
  ...
  public static async create({
    ...
    let currentBlock;
    if (await native.Ledger.isInitialized(storage)) {
  ...
  // TODO: should this come directly from Ledger contract storage?
  // And should currentBlockIndex come from Ledger?
  ...
  private updateBlockMetadata(block: Block): void {
    ...
    this.headerCache.add(block.header);
  }
  ...
  private async persistBlockInternal(block: Block, verify?: boolean): Promise<void> {
    ...
    if (firstHeader !== undefined && firstHeader.index !== block.index) {
      logger.error({ name: 'neo_blockchain_header_cache_does_not_match_block_index' });
    }
  }
  ...
  private createDummyBlock(): Block {
    // TODO: use these or make sure blockchain getters use Ledger contract
    // const hash = this.native.Ledger.currentHash(storage);
    // const currentBlock = this.native.Ledger.getBlock(storage, hash);

    return new Block({
      version: 0,
      previousHash: this.currentBlock.hash, // TODO might have to change this
      timestamp: this.currentBlock.timestamp.addn(this.settings.millisecondsPerBlock),
      index: this.currentBlockIndex + 1, // TODO: might need to change this
